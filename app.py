...
def load_and_score_data():
    ...
    def classify_status(row):
        ...
        # UNIFIED LOGIC: If Wear AND Torque are high, it is strictly AT RISK
        if row['Tool_Wear_min'] >= wear_threshold and row['Torque_Nm'] >= torque_threshold:
            return "At Risk"
        return "Operational"
...
with col3:
    # Stylized metric to match Capital Exposed
...
with left_col:
    ...
    st.info(f"""
    STRATEGIC INSIGHT: The yellow cluster represents assets converging on the critical 
    failure threshold. Risk is mathematically confirmed when Tool Wear exceeds {w_thresh:.0f} mins 
    and Torque exceeds {t_thresh:.1f} Nm. Assets in this quadrant require prioritized isolation.
    """)
    
    # 3D Scatter (Optimized Color Mapping)
    plot_df = df.head(1000)
    fig = px.scatter_3d(plot_df, 
                        x='Tool_Wear_min', y='Torque_Nm', z='Air_Temp_K',
                        color='Status',
                        color_discrete_map={
                            "Historical Failure": "#FF0000", # Pure Red
                            "At Risk": "#FFD700",           # Strategic Gold
                            "Operational": "#0000FF"        # Professional Blue
                        },
                        opacity=0.7,
                        labels={
    ...
    fig.update_layout(margin=dict(l=0, r=0, b=0, t=0), legend_title_text='Asset Classification')
